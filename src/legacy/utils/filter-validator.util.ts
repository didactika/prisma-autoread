...
export default class FilterValidator {
    ...
    /**
     * Return the scalar Prisma type of a field using case-insensitive matching.
     * @param fieldName - Incoming field name (may differ in casing)
     * @param modelInfo - DMMF model object
     * @returns The Prisma type (e.g. `'String'`, `'Int'`, `'Json'`), or `null` if not a scalar/enum field
     */
    static getScalarFieldType(fieldName: string, modelInfo: any): string | null {
        if (!modelInfo?.fields) return null;

        const lowerFieldName = fieldName.toLowerCase();
        const field = modelInfo.fields.find(
            (f: any) => f.name.toLowerCase() === lowerFieldName && f.kind !== 'object'
        );
    ...
    /**
     * Detect whether a field is a `Json` column (case-insensitive) and return its
     * correctly-cased name so the caller can address it as a JSON filter.
     *
     * @param fieldName - Incoming field name (may differ in casing)
     * @param modelInfo - DMMF model object
     * @returns The correctly-cased field name if it is a `Json` field, otherwise `null`
     */
    static isJsonField(fieldName: string, modelInfo: any): string | null {
        if (!modelInfo?.fields) return null;
