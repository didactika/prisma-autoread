...
import { LikeFilter, JsonFilter } from '../../types';
...
export default class NestedRelationProcessor {
     ...
     * @param jsonFilters - Optional array to accumulate `Json` column filters
     */
    static processString(
        ...
        for (let i = 0; i < fieldParts.length; i++) {
            ...
            // Json column → every remaining segment is a path inside the document.
            const jsonName = FilterValidator.isJsonField(partName, currentModelInfo);
            if (jsonName) {
                jsonFilters?.push({
                    field: [...relationPath, jsonName].join('.'),
                    path: fieldParts.slice(i + 1),
                    mode: explicitMode ?? 'EXACT',
